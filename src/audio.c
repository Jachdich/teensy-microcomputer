...
bool sgtl5000_setup(void) {
    Wire_begin();
    delay(5);
    bool r = sgtl5000_write(CHIP_ANA_POWER, ANA_POWER_REFTOP_POWERUP | ANA_POWER_ADC_MONO | ANA_POWER_DAC_MONO);
    if (!r) return false;
    sgtl5000_write(CHIP_LINREG_CTRL, LINREG_VDDC_VOLTAGE(0x0C /*1.0V*/) | LINREG_VDDC_FROM_VDDIO | LINREG_VDDC_SOURCE_OVERRIDE);
    sgtl5000_write(CHIP_REF_CTRL, REF_VAG_VOLTAGE(0x1F /*1.575V*/) | REF_BIAS_CTRL(0x1 /*+12.5%*/));
    sgtl5000_write(CHIP_LINE_OUT_CTRL, LINE_OUT_CTRL_VAG_VOLTAGE(0x22 /*1.65V*/) | LINE_OUT_CTRL_OUT_CURRENT(0x3C /*0.54mA*/));    
    sgtl5000_write(CHIP_SHORT_CTRL, 0x4446); // fuck it, I cba to work this out
  	sgtl5000_write(CHIP_ANA_CTRL, 0x0137);  // enable zero cross detectors
		...
		sgtl5000_write(CHIP_ANA_POWER,
            ANA_POWER_CAPLESS_HEADPHONE_POWERUP
          | ANA_POWER_DAC_POWERUP
          | ANA_POWER_HEADPHONE_POWERUP
          | ANA_POWER_DAC_MONO
          | ANA_POWER_VAG_POWERUP
          | ANA_POWER_REFTOP_POWERUP);
